...
class CachingSideViewController: UITableViewController, SideMenuControllerDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.text = "Switch to: " + (dataSource[(indexPath as NSIndexPath).row] as! CacheableViewController.Type).cacheIdentifier
        return cell!
    }
    
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let controllerType = dataSource[(indexPath as NSIndexPath).row] as! CacheableViewController.Type
